    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-500 mt-0.5">Verifique las directrices de seguridad física del Segmento 1 y ejecute análisis de integridad de red.</p>
        </div>

        <button
          onClick={runSystemAudit}
          disabled={runningAudit}
          className="flex items-center gap-2 px-4 py-2.5 bg-white hover:bg-slate-50 border border-[#8DC63F]/50 hover:border-[#8DC63F] rounded-xl text-xs font-bold text-slate-700 tracking-wider transition-all cursor-pointer shadow-md disabled:opacity-50 shrink-0"
        >
          {runningAudit ? (
            <div className="w-4 h-4 border-2 border-slate-300 border-t-[#8DC63F] rounded-full animate-spin" />
          ) : (
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <div className="p-5 rounded-2xl glass-panel flex items-center justify-between glass-panel-hover">
        ...
        <div className="p-5 rounded-2xl glass-panel flex items-center justify-between glass-panel-hover">
          <div className="space-y-1.5">
            ...
            <span className="text-[10px] text-emerald-600 font-bold">Políticas institucionales activadas</span>
          </div>
            ...
            <div className="p-6 space-y-5">
              ...
              <div className="grid grid-cols-2 gap-4 p-4 rounded-xl bg-slate-50 border border-slate-100 text-xs">
                ...
                <div>
                  ...
                  <span className="text-xl font-black text-slate-700 font-['Outfit']">{auditReport.scannedDevices} Lectores QR</span>
                </div>
                  ...
                  {auditReport.findings.map((f, i) => (
                    <div key={i} className={`p-3.5 rounded-xl border text-xs flex gap-3 items-start ${
                      ...
                      <span className={`font-mono text-[9px] font-bold uppercase px-1.5 py-0.5 rounded border shrink-0 mt-0.5 ${
                         f.type === 'warning' ? 'bg-red-100 border-red-300 text-red-600' : 'bg-slate-50 border-slate-200 text-slate-500'
                      }`}>
                        {f.code}
                      </span>
              ...
              <div className="flex items-center justify-end pt-4 border-t border-slate-100">
                <button
                  ...
                  className="px-4 py-2.5 bg-slate-100 hover:bg-slate-200 border border-slate-200 rounded-xl text-xs font-bold text-slate-600 transition-all cursor-pointer"
                >
                  Cerrar Diagnóstico
                </button>
              </div>
